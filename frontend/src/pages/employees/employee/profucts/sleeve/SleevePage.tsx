...
const SleevePage = () => {
  ...
  const handleDelete = async (category: DataForm) => {
    const result = await Swal.fire({
      title: "¿Estás seguro?",
      text: `Eliminarás el tipo de cuello "${category.name}". Esta acción no se puede deshacer.`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        const response = await deleteSleeve(category.id);
        if (response) {
          setItems((prev) => prev.filter((cat) => cat.id !== category.id));

          Swal.fire({
            title: "Eliminado",
            text: `El tipo de cuello "${category.name}" ha sido eliminado.`,
            icon: "success",
            confirmButtonColor: "#2563EB",
          });
        } else {
      ...
      <div className="bg-white dark:bg-gray-800 w-full max-w-7xl rounded-xl shadow-lg p-6 mb-6">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">
            Administra el tipo de cuello de productos fácilmente.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8 w-full max-w-7xl">
        ...
        <div className="md:w-1/3 h-64 bg-white dark:bg-gray-700 p-6 rounded-lg shadow-md flex flex-col">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-4">
            {editId !== null ? "Editar Cuello" : "Agregar Cuello"}
          </h2>
          <form
