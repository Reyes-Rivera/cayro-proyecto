...
const SizePage = () => {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    try {
      if (editId !== null) {
        setIsLoading(true);
        const updatedItem = await updateSize(editId, data);
        if (updatedItem) {
          Swal.fire({
            icon: "success",
        ...
        setIsLoading(true);
        const newItem = await addSize(data);
        if (newItem) {
          Swal.fire({
            icon: "success",
  ...
  const handleEdit = (size: DataForm) => {
    setValue("name", size.name);
    setEditId(size.id);
    setIsModalOpen(true);
  };

  const handleDelete = async (size: DataForm) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response) {
          setItems((prev) => prev.filter((cat) => cat.id !== size.id));
  ...
  // Abrir modal para agregar nueva talla
  const openAddModal = () => {
    setEditId(null);
    reset();
    setIsModalOpen(true);
              ...
              <div className="flex items-start gap-3 sm:gap-5">
                ...
                <div>
                  ...
                  <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300 mt-1 sm:mt-2 max-w-2xl">
                    Administra las tallas de los productos de tu catálogo. Las
                    tallas son esenciales para la correcta selección de
                    productos.
                  </p>
                </div>
              </div>
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800/30 flex flex-col xs:flex-row items-start xs:items-center justify-between gap-2 xs:gap-0">
          <div className="flex items-center gap-2">
            <div className="bg-blue-100 dark:bg-blue-800/50 p-1.5 rounded-lg">
              <Ruler className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600 dark:text-blue-400" />
            </div>
            <span className="text-xs sm:text-sm font-medium text-blue-700 dark:text-blue-300">
              {filteredAndSortedItems.length}{" "}
              {filteredAndSortedItems.length === 1 ? "talla" : "tallas"} en
              total
            </span>
          </div>
              ...
              {currentItems.length > 0 ? (
                        ...
                        <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
                          ...
                          <div>
                            ...
                            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">
                              Talla registrada en el sistema
                            </p>
                          </div>
                        </div>
                ...
                >
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-4 sm:p-6 rounded-full mb-4">
                    <Ruler className="w-8 h-8 sm:w-12 sm:h-12 text-blue-400 dark:text-blue-300" />
                  </div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-4 sm:mb-6 max-w-md">
                    {searchTerm
                      ? "Intenta con otro término de búsqueda o limpia los filtros para ver todas las tallas"
                      : "Añade tallas para comenzar a gestionar tu catálogo de productos"}
                  </p>
                  {searchTerm ? (
                    <motion.button
                      whileHover={{ scale: 1.05 }}
        ...
        {filteredAndSortedItems.length > 0 && (
          ...
          >
            ...
            <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 text-center">
              ...
              {filteredAndSortedItems.length} tallas
            </div>
                      ...
                      <input
                        {...register("name", {
                          ...
                          minLength: {
                            value: 1,
                            message:
                              "El nombre de la talla debe tener al menos un caracter",
                          },
                          maxLength: {
                            value: 50,
                            message:
                              "El nombre de la talla no puede exceder los 50 caracteres",
                          },
                          pattern: {
                            value: /^[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]+$/,
                            message:
                              "El nombre de la talla solo puede contener letras y números.",
                          },
                        })}
                        id="sizeName"
                        type="text"
