...
const BrandPage = () => {
    ...
    try {
      if (editId !== null) {
        ...
        if (updatedItem) {
          Swal.fire({
            icon: "success",
            title: "Tipo de marca actualizado",
            text: "El tipo de marca ha sido actualizado exitosamente.",
        ...
        if (newItem) {
          Swal.fire({
            icon: "success",
            title: "Tipo de marca agregado",
            text: "El tipo de marca ha sido agregado exitosamente.",
            confirmButtonColor: "#2563EB",
          });
  ...
  const handleDelete = async (category: DataForm) => {
    const result = await Swal.fire({
      title: "¿Estás seguro?",
      text: `Eliminarás el tipo de marca "${category.name}". Esta acción no se puede deshacer.`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        const response = await deleteBrand(category.id);
        if (response) {
          setItems((prev) => prev.filter((cat) => cat.id !== category.id));

          Swal.fire({
            title: "Eliminado",
            text: `El tipo de marca "${category.name}" ha sido eliminado.`,
            icon: "success",
            confirmButtonColor: "#2563EB",
          });
        } else {
        ...
        Swal.fire({
          title: "Error",
          text: "Ocurrió un problema al eliminar el tipo de marca. Inténtalo de nuevo.",
          icon: "error",
          confirmButtonColor: "#d33",
        });
      }
      ...
      <div className="bg-white dark:bg-gray-800 w-full max-w-7xl rounded-xl shadow-lg p-6 mb-6">
        <div className="flex flex-col items-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">
            Administra el tipo de marca de los productos fácilmente.
          </p>
        </div>
      </div>
            ...
            <tbody>
              {items.length > 0 ? (
                ...
                <tr>
                  ...
                  >
                    No hay tipos de marca disponibles.
                  </td>
                </tr>
              )}
            </tbody>
