...
import { recomendation } from "@/api/products";
import type { Product } from "@/types/products";

interface ProductRecommendationsCarouselProps {
  currentProduct: Product;
}

const ProductRecommendationsCarousel = ({
  currentProduct,
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      ...
      try {
        ...
        // Filter current product and products without variants
  ...
  if (isLoading) {
    return (
      <section className="mt-16 mb-8 bg-gradient-to-br from-gray-50 via-indigo-50/20 to-purple-50/10 dark:from-gray-900 dark:via-indigo-950/20 dark:to-purple-950/10 relative overflow-hidden rounded-2xl p-8">
        ...
        <div className="relative z-10">
          <div className="text-center mb-8 md:mb-12">
            <span className="inline-flex items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-900/30 px-4 py-1.5 text-sm font-medium text-indigo-800 dark:text-indigo-300 mb-4">
            ...
            <div className="h-1 w-24 bg-indigo-600 mx-auto mt-4 md:mt-6"></div>
          </div>
  ...
  if (error) {
    return (
      <section className="mt-16 mb-8 bg-gradient-to-br from-gray-50 via-red-50/20 to-orange-50/10 dark:from-gray-900 dark:via-red-950/20 dark:to-orange-950/10 relative overflow-hidden rounded-2xl p-8">
        {/* Background decoration */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
    ...
    >
      ...
      <div className="relative z-10">
        <div className="text-center mb-8 md:mb-12 opacity-100 translate-y-0">
          ...
          <div className="h-1 bg-indigo-600 mx-auto mt-4 md:mt-6 w-24"></div>
        ...
        <div className="relative">
          ...
          {recommendations.length > itemsToShow && (
            <>
              <button
                onClick={prevSlide}
                className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-6 z-10 bg-white dark:bg-gray-800 text-gray-800 dark:text-white p-2 md:p-3 rounded-full shadow-lg hover:bg-indigo-600 hover:text-white transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2"
                aria-label="Anterior"
              >
              ...
              <button
                onClick={nextSlide}
                className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-6 z-10 bg-white dark:bg-gray-800 text-gray-800 dark:text-white p-2 md:p-3 rounded-full shadow-lg hover:bg-indigo-600 hover:text-white transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2"
          ...
          <div className="text-center mt-4">
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {recommendations.length} productos recomendados
            </span>
          </div>
        </div>
      </div>
...
export default memo(ProductRecommendationsCarousel);
