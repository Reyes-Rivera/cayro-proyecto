...
export default function LegalBoundary() {
  ...
  const fetchDocument = useCallback(async () => {
    ...
    try {
      ...
      // Admite res.data como objeto o arreglo
  ...
  const parsedSections = useMemo(() => {
    ...
    return content.split("\n").map((line) => {
      const m = line.match(/^(.+?):$/);
      if (m) return { type: "heading" as const, text: m[1] + ":" };
      if (line.trim() === "") return { type: "spacer" as const, text: "" };
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="prose prose-lg max-w-none dark:prose-invert prose-headings:text-gray-900 dark:prose-headings:text-white prose-p:text-gray-700 dark:prose-p:text-gray-300">
                        <AnimatePresence>
                          {parsedSections.map((blk, index) => {
                            ...
                            // spacer
                            return <div key={`s-${index}`} className="my-4" />;
                          })}
                        </AnimatePresence>
                      </div>
                      ...
                      <div className="mt-8 flex items-center gap-3">
                        <button
                          ...
                          className="px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors text-sm"
                          aria-label="Recargar documento"
                        >
                          Recargar documento
                        </button>
                      </div>
