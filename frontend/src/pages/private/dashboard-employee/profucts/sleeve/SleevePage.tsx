...
const SleevePage = () => {
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    try {
      ...
      if (editId !== null) {
        const updatedSleeve = await updateSleeve(editId, data);
        if (updatedSleeve) {
          AlertHelper.success({
            title: "Tipo de cuello actualizado",
            message: "El tipo de cuello ha sido actualizado exitosamente.",
          });
        ...
        const newSleeve = await addSleeve(data);
        if (newSleeve) {
          AlertHelper.success({
            title: "Tipo de cuello agregado",
            message: "El tipo de cuello ha sido agregado exitosamente.",
          });
      ...
      AlertHelper.error({
        title: "Error",
        error,
        message: "Ha ocurrido un error al guardar el tipo de cuello.",
        animation: "fadeIn",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleEdit = (sleeve: Sleeve) => {
    setValue("name", sleeve.name);
    setEditId(sleeve.id);
    setShowStatusModal(true);
  };

  const handleDelete = async (sleeve: Sleeve) => {
    const confirmed = await AlertHelper.confirm({
      title: "¿Estás seguro?",
      message: `Eliminarás el tipo de cuello "${sleeve.name}". Esta acción no se puede deshacer.`,
    ...
    try {
      ...
      if (response) {
        setSleeves((prev) => prev.filter((cat) => cat.id !== sleeve.id));

        AlertHelper.success({
          title: "Eliminado",
          message: `El tipo de cuello "${sleeve.name}" ha sido eliminado.`,
        });
      } else {
      ...
      AlertHelper.error({
        title: "Error",
        error,
        message: "Ha ocurrido un error al eliminar el tipo de cuello.",
        animation: "fadeIn",
      });
    }
  };
  ...
  const filteredAndSortedSleeves = sleeves
    .filter((sleeve) =>
      sleeve.name.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      const aValue = a[sortBy.value as keyof Sleeve];
      const bValue = b[sortBy.value as keyof Sleeve];
  ...
  const currentItems = filteredAndSortedSleeves.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="mt-1 text-white/80 flex items-center text-sm sm:text-base">
                  <Shirt className="w-3 h-3 sm:w-3.5 sm:h-3.5 mr-1.5 inline" />
                  {sleeves.length}{" "}
                  {sleeves.length === 1 ? "tipo de cuello" : "tipos de cuello"}{" "}
                  en el catálogo
                </p>
              </div>
            </div>
              ...
              <div className="divide-y divide-gray-100 dark:divide-gray-700">
                {currentItems.length > 0 ? (
                      ...
                      <div className="sm:hidden flex flex-col space-y-3">
                        ...
                        <div className="flex justify-end space-x-2">
                          <button
                            ...
                            className="p-2 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-800/50 transition-colors"
                            title="Editar tipo de cuello"
                            aria-label="Editar tipo de cuello"
                          >
                            <Edit size={16} />
                          </button>
                          <button
                            ...
                            className="p-2 rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-800/50 transition-colors"
                            title="Eliminar tipo de cuello"
                            aria-label="Eliminar tipo de cuello"
                          >
                            <Trash size={16} />
                          </button>
                        </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-center px-4">
                    <Shirt className="h-12 w-12 text-gray-300 dark:text-gray-600 mb-3" />
                    <p className="text-lg font-medium text-gray-700 dark:text-gray-300">
                      {searchTerm
                        ? "No se encontraron tipos de cuello"
                        : "No hay tipos de cuello"}
                    </p>
                    <p className="text-sm mb-4 text-gray-500 dark:text-gray-400">
                      {searchTerm
                        ? "No hay tipos de cuello que coincidan con la búsqueda"
                        : "Comienza creando un nuevo tipo de cuello"}
                    </p>
                    <div className="mt-6">
                      {searchTerm ? (
                        <button
              ...
              {totalPages > 1 && (
                <div className="bg-gray-50 dark:bg-gray-800/50 p-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4">
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    {Math.min(indexOfLastItem, filteredAndSortedSleeves.length)}{" "}
                    de {filteredAndSortedSleeves.length} tipos de cuello
                  </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto mx-4">
            <div className="p-6">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                {editId !== null
                  ? "Modifica el nombre del tipo de cuello seleccionado"
                  : "Ingresa el nombre del nuevo tipo de cuello para tu catálogo"}
              </p>
                  ...
                  <input
                    {...register("name", {
                      ...
                      minLength: {
                        value: 1,
                        message:
                          "El nombre del tipo de cuello debe tener al menos 1 caracter",
                      },
                      maxLength: {
                        value: 50,
                        message:
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  >
                    {isLoading ? (
                      ...
                      "Actualizar Tipo de Cuello"
                    ) : (
                      "Crear Tipo de Cuello"
                    )}
                  </button>
                </div>
              </form>
