...
// Usar las mismas interfaces del componente principal
...
interface DocumentDialogProps {
  ...
  onClose: () => void;
  addDocument: (
    newDocument: Omit<RegulatoryDocument, "id" | "createdAt" | "status">
  ) => void;
  updateDocument?: (updatedDocument: RegulatoryDocument) => void;
  documentToEdit?: RegulatoryDocument | null;
}
...
// Resto del código del DocumentDialog permanece igual...
...
// Componentes reutilizables (mantener igual)
...
export default function DocumentDialog({
  ...
  useEffect(() => {
    if (documentToEdit) {
      ...
      setValue("type", documentToEdit.type || DocumentTypeInter.policy);
    }
    ...
    async (data) => {
      ...
      try {
        ...
        if (documentToEdit && updateDocument) {
          const updatedDocument: RegulatoryDocument = {
            ...documentToEdit,
            ...data,
          ...
          >
            ...
            <div className="flex justify-end mt-8 gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
              <motion.button
                ...
                onClick={onClose}
                disabled={isLoading}
                className="px-6 py-2.5 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-medium rounded-xl border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 transition-all shadow-sm disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-gray-500"
              >
                Cancelar
              </motion.button>
              <motion.button
