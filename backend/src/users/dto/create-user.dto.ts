...
export class CreateUserDto {
  ...
  @Matches(/^(?!.*[<>])^\S+@\S+\.\S+$/, {
    message:
      'El correo electrónico no debe contener los caracteres "<" o ">", ni espacios.',
  })
  email: string;
  ...
  @Matches(
    ...
    {
      message:
        'La contraseña debe contener al menos una minúscula, una mayúscula, un número y un carácter especial (!@#$%^&*(),.?:{}|). No puede contener "<" o ">".',
    },
  )
  password: string;
...
export class AnswerQuestion {
  ...
  @MaxLength(250, {
    message: 'La respuesta no debe superar los 250 caracteres.',
  })
  @Matches(/^[a-zA-ZÀ-ÿ\u00f1\u00d1\s]+$/, {
    message:
      'Por favor, ingrese solo letras (mayúsculas o minúsculas), espacios y caracteres acentuados.',
  ...
  @IsNumber()
  @IsNotEmpty({
    message: 'El id debe ser de tipo número y no puede estar vacío.',
  })
  securityQuestionId: number;

  @IsString({ message: 'El correo electrónico debe ser un texto válido.' })
  @IsOptional({ message: 'El correo electrónico es requerido.' })
  @IsEmail({}, { message: 'Por favor, ingrese un correo electrónico válido.' })
  @MinLength(10, { message: 'El correo debe tener al menos 10 caracteres.' })
  @MaxLength(50, { message: 'El correo no debe superar los 50 caracteres.' })
  @Matches(/^(?!.*[<>])^\S+@\S+\.\S+$/, {
